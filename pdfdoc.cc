...
void PDFDoc::DrawPage(SkCanvas* canvas, SkRect rect, int pageno) {
  ...
  ScopedPage page = pagecache_.OpenPage(this, pageno);

  // Draw each annotation on this page
  for (ssize_t i = page.AnnotationCount() - 1; i >= 0; i--) {
    Annotation* annot = page.GetAnnotation(i);
    annot->Draw(canvas, rect);
  }
}

void PDFDoc::RenderPage(SkCanvas* canvas, SkRect rect, int pageno) {
  ...
  // Render the page
  ScopedPage page = const_cast<PageCache&>(pagecache_).OpenPage(this, pageno);
  if (!page) {
    fprintf(stderr, "failed to load PDFPage\n");
    return;
  }
...
const std::vector<std::unique_ptr<Annotation>>& PDFDoc::AnnotationsOnPage(
    int pageno) {
  ScopedPage page = pagecache_.OpenPage(this, pageno);
  return page.Annotations();
}

void PDFDoc::PushAnnotation(int pageno, std::unique_ptr<Annotation> annot) {
  ScopedPage page = pagecache_.OpenPage(this, pageno);
  page.PushAnnotation(std::move(annot));
}

void PDFDoc::AnnotationsMovedUndo(int pageno, const std::set<Annotation*>& annots,
                                  float dx, float dy) {
  ScopedPage page = pagecache_.OpenPage(this, pageno);
  page.MarkDirty();
  for (Annotation* annot : annots) {
    for (PDFDocEventHandler* handler : event_handlers_) {
      // Compute old rect and new rect
      SkRect bounds = annot->Bounds();
      handler->NeedsDisplayInRect(pageno, bounds);  // new location
      bounds.offset(-dx, -dy);
      handler->NeedsDisplayInRect(pageno, bounds);  // old location
    }
  }
  // TODO(adlr): generate undo op
}

std::unique_ptr<txt::Paragraph> PDFDoc::ParseText(const char* text) {
